...
This next section runs the bulk of the calculations. This is the most time consuming step and will not be evaluated as part of this code chunk for time. The generated file will be loaded in the next step, but is calculated with the following lines. 
...
Figure 1 plot generation. We create the heatmap and analyze the mismatch differences across various serovars. 
...
Figure 2 Generation - Typhimurium Section

```{r Figure 2 Typhimurium}
#Locating Typhimurium Clade
load("/Users/dogrinev/Desktop/Tree.Rdata")
which(vert.tree.SE_Final$tip.label=="GCA_000213635.1")
Ancestors(x = vert.tree.SE_Final,node = 249)
Descendants(x = vert.tree.SE_Final,node = 1642)

Tree_Subset<-unlist(Descendants(x = vert.tree.SE_Final,node = 1642))
Vec<-1:1565
Tree_Subset2<-Vec[-Tree_Subset]
vert.tree.cut<-ape:::drop.tip(phy = vert.tree.SE_Final,tip = Tree_Subset2)

SerovarTable_Cut<-SerovarTable_SE3[which(SerovarTable_SE3$Serovar=="Typhimurium/Monophasic"),]
...
Figure 3 Kentucky Section

```{Figure 3 Kentucky}
#Locating Kentucky Clade
which(vert.tree.SE$tip.label=="GCA_018340485.1")
Ancestors(x = vert.tree.SE,node = 640)
Descendants(x = vert.tree.SE,node = 2331)
#Node 2331 appears to be a good starting point for locating the Kentucky clade.

#Next we trim out tree parts to cut down to visualizing Kentucky only.
Tree_Subset<-unlist(Descendants(x = vert.tree.SE,node = 2331))
Vec<-1:1700
Tree_Subset2<-Vec[-Tree_Subset]
vert.tree.cut<-ape:::drop.tip(phy = vert.tree.SE,tip = Tree_Subset2)
Cut<-unlist(Descendants(x = vert.tree.cut,node = 331))
vert.tree.cut2<-drop.tip(phy = vert.tree.cut,tip = Cut)
Cut2<-unlist(Descendants(x = vert.tree.cut2,node = 167))
vert.tree.cut3<-drop.tip(phy = vert.tree.cut2,tip = Cut2)

#Plotting Kentucky Serovar
SerovarTable_Cut<-SerovarTable_SE2[which(SerovarTable_SE2$Serovar=="Kentucky"),]
...
for(x in Run)
{
  ...
  Concat_Orders<-map(.x = 1:1564,.f = ~Super_HAM_Placer2(TestSequence = TestSequence,ReferenceSequence = Full_16s_Data_Aligned2[[.]]))
  Concatenated_References<-map(.x = 1:1564,.f = ~Concatenate_By_Order(Order = Concat_Orders[[.]][1:7],RefSequence = Full_16s_Data_Aligned2[[.]]))
...
Clade_Hit_Finder<-function(Original,Pendant_Multi)
{
  ...
  vert.tree.trimmed<-ape:::drop.tip(phy = vert.tree.SE,tip = x)
  ...
  TipNumbersOI_OriginalTree<-which(vert.tree.SE$tip.label %in% TipNamesOI_Filtered)
  Edge.Table<-cbind(vert.tree.SE$edge,vert.tree.SE$edge.length)
  Ancestor_List<-map(.x = TipNumbersOI_OriginalTree,.f = ~unlist(Ancestors(vert.tree.SE,node = .)))
  ...
  Root_Node<-rootnode(vert.tree.SE)
  ...
  if(Closest_Parent_Node != Root_Node)
  {
    Potential_Ancestors<-Ancestors(x = vert.tree.SE,node = Closest_Parent_Node)
...
Placement_Method_Evaluator<-function(Number)
{
  ...
  Placement_Accuracy<-map(.x = 1:1441,.f = ~Clade_Accuracy(MRCA = PlacementMappingData_MRCAs_Pendant_New[[.]],Original = FullTestQueries[.]))
  ...
  Placement_Checker<-map(.x = 1:1441,.f = ~Clade_Checker(MRCA = PlacementMappingData_MRCAs_Pendant_New[[.]],Original = FullTestQueries[.]))
  Placement_Checker_DF<-as.data.frame(matrix(unlist(Placement_Checker), ncol = 2, byrow = TRUE))
  Originals_Found<-length(which(Placement_Checker_DF$V1==6))/1441
...
{
  Descendant_List<-Descendants(x = vert.tree.SE,node = MRCA)
  Descendants_Vec<-unlist(Descendant_List)
  Descendant_Assemblies<-vert.tree.SE$tip.label[Descendants_Vec]
  Original_Serovar<-SerovarTable_SE2[which(SerovarTable_SE2$Assembly %in% Original),2]
  Descendant_Serovars<-SerovarTable_SE2[which(SerovarTable_SE2$Assembly %in% Descendant_Assemblies),2]
  ...
  Node_Distances<-dist.nodes(vert.tree.SE)
...
{
  Descendant_List<-Descendants(x = vert.tree.SE,node = MRCA)
  Original_Node<-which(vert.tree.SE$tip.label==Original)
  ...
  {
    Node_Distances<-dist.nodes(x = vert.tree.SE)
...
Determine MRCAs - These are calculated from the placement JSON files. 

```{r MRCA Calculation, eval=FALSE}
Original_Node_Distances<-dist.nodes(vert.tree.SE)
...
Calculation of our predictions and the actual results.
...
for(i in 1:1441)
{
  ...
  GTD_Sero_Predict_Clean2<-SerovarTable_SE
...
for(i in 1:1441)
{
  ...
  QuerySero<-SerovarTable_SE2[which(SerovarTable_SE2$Assembly==Query),2]
...
Generation of results summary (Figure 4)
...
Visualization of Full Salmonella Results
...
#Nice barplot:
serosize<-names(sort(table(SerovarTable_SE$Serovar),decreasing = TRUE))
